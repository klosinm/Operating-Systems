    ...
    ***************************************/
    ...
    while (1)
    {
        ...
        else if (processNum == ProcessDelivery)
        {
            ...
            printf("Process %i (%d)-(PPID:%d) DELIEVERY! read message: %s", processNum, getpid(), getppid(), readMessage);
        ...
        {
            read(pipes[processNum - 1][READ], readMessage, sizeof(readMessage));
            printf("Process %i (%d)-(PPID:%d) read message: %s", processNum, getpid(), getppid(), readMessage);
